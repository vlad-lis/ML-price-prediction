...
X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=tt_ratio, random_state=rand_seed)
    ...
    while True:
        ...
        try:
            if int(prompt) not in options:
                print(
                    f' ! pick something between {min(options)}'
                    f' and {max(options)}')
            else:
                return prompt
        except ValueError:
...
def prediction():
    ...
    ******************************
    ...
    if price == 0:
        output = 'Sorry, but the model says this car is a worthless :('
    else:
